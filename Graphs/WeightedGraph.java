...
public class WeightedGraph<T> implements WeightedGraphInterface<T>
{
  ...
  {
	  int idxFrom = indexIs(fromVertex);
	  int idxTo = indexIs(toVertex);
	  edges[idxFrom][idxTo] = weight;
  }
  ...
  {
	  boolean[] tempMarks = new boolean[maxVertices];
	  marks = tempMarks;
  }
  ...
  // Marks vertex.
  {
	  int vertexIdx = indexIs(vertex);
	  marks[vertexIdx] = true;
  }
